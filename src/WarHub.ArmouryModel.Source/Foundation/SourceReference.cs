...
public abstract class SourceReference
{
    ...
    public abstract SourceNode GetNode(CancellationToken cancellationToken = default);
    ...
    /// </summary>
    /// <returns>The original referenced syntax node.</returns>
    public virtual Task<SourceNode> GetNodeAsync(CancellationToken cancellationToken = default)
    {
        return Task.FromResult(GetNode(cancellationToken));
    }
    ...
    {
        return SourceTree.GetLocation(Span);
    }
}
